...
class distance_to_ball(RewardFunction):
    def __init__(self):
        super().__init__()
    ...
    def get_reward(self, player: PlayerData, state: GameState, previous_action: np.ndarray) -> float:
        ...
        reward = (player.facing_ball-.5)*3 + player.ball_touched*130 + (np.dot(math.unitvec(player.car_data.linear_velocity), math.unitvec(distance_ball_car))-.3)*velocity_num/15 - 20/(velocity_num+1) - 3
